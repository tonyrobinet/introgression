...
################################
## PSI: RANGE EXPANSION ANALYSIS
################################

## requires re_functions (at the end of the script) to be entirely loaded
...
# setwd("~/R/x86_64-pc-linux-gnu-library/3.1/rangeExpansion")
# Copy/paste the code of the file re_functions.r

snp.file <- "~/sync/poissons/genetique_poissons/genotypage_bar/resultats/analyses_finales/PSI_range_expansion/Merged_827_Chips_14_Genomes_1012_SNPs_NoRep.bed"
coord.file <- "~/sync/poissons/genetique_poissons/genotypage_bar/resultats/analyses_finales/PSI_range_expansion/coords_file_no_outgroup_NORD_CS_CH_IS_NS.csv"
ploidy <- 2 #diploid individuals
region <- list("PORT","BISC","NORD",c("PORT","BISC","NORD"))
raw.data <- load.data(plink.file=snp.file, coords.file=coord.file, sep=',', ploidy=ploidy) # error...

pop <- make.pop(raw.data, ploidy)
psi_no_out <- get.all.psi(pop)
write.table(psi_no_out,"psi_no_outgroup_NORD_CS_CH_IS_NS.txt",quote=F)

results <- run.regions(region=region, pop=pop, psi=psi_no_out, xlen=100, ylen=100)
plot(results)
summary(results)
...
plot.origin.results <- function(x, n.levels=100, color.function=heat.colors,
  ...
  if(add.likely.origin){
    points(summary(x)[,1:2], col='blue', pch='x', cex=3)
  }
  if(add.map){
    require(rworldmap)
    m <- getMap("high")
